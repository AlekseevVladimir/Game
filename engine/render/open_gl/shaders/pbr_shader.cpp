...
void PBRShader::configure() {
	...
	for (int i = 0; i < ptrsVector.size(); ++i) {
		...
		glm::vec3 color(150.f, 150.f, 150.f);
		setFloat3((baseStr + std::string("].color")).c_str(),
			&color[0]);
...
void PBRShader::setMatrices(GameObject* viewPointPtr)
{
...
void PBRShader::setTextureData(std::vector<Mesh::Texture>& textures) 
{
	TexturesCtrl& texCtrl = TexturesCtrl::getInstance();
	...
	for (const Mesh::Texture& texture : textures)
	{
		...
		setInt1(("uMaterial." + name).c_str(), idx);
